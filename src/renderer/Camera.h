...
requires m::is_mat<t_mat> && m::is_vec<t_vec> && m::is_vec<t_vec3>
class Camera
{
public:
	Camera() = default;
	...
	{
		m_mat = m::hom_lookat<t_mat, t_vec3>(
			pos, target, up);

		std::tie(m_mat_inv, std::ignore)
			= m::inv<t_mat, t_vec>(m_mat);
	...
	void Rotate(t_real dphi, t_real dtheta, bool restrict_theta = true)
	{
		...
		m_phi = m::mod_pos<t_real>(
			m_phi, t_real(2)*m::pi<t_real>);

		// wrap around theta angle
		m_theta = std::fmod(m_theta, t_real(2)*m::pi<t_real>);

		// restrict theta angle
		if(restrict_theta)
			m_theta = std::clamp<t_real>(
				m_theta, -t_real(0.5)*m::pi<t_real>, 0.);
	...
	{
		t_vec3 xdir = m::row<t_mat, t_vec3>(m_matRot, 0);
		t_vec3 ydir = m::row<t_mat, t_vec3>(m_matRot, 1);
		t_vec3 zdir = m::row<t_mat, t_vec3>(m_matRot, 2);
	...
	{
		auto [ persp, vec ] =
			m::hom_to_screen_coords<t_mat, t_vec>(
		...
		const std::vector<t_vec>& bbox) const
	{
		...
		auto [min, max] = m::minmax_comp(brect);

		return std::vector<t_vec>{{
			m::create<t_vec>({min[0], min[1], 0., 1.}),
			m::create<t_vec>({min[0], max[1], 0., 1.}),
			m::create<t_vec>({max[0], max[1], 0., 1.}),
			m::create<t_vec>({max[0], min[1], 0., 1.}),
		}};
	}
	...
	std::tuple<t_vec3, t_vec3> GetPickerRay(t_real mouseX, t_real mouseY) const
	{
		...
		t_vec3 org3 = m::create<t_vec3>({org[0], org[1], org[2]});
		t_vec3 dir3 = m::create<t_vec3>({dir[0], dir[1], dir[2]});
...
//protected:
	...
	void UpdateTransformation()
	{
		...
		{
			m::create<t_vec3>({1., 0., 0.}),
			m::create<t_vec3>({0. ,0., 1.})
		};

		m_matRot = m::hom_rotation<t_mat, t_vec3>(
			vecCamDir[0], m_theta, 0);
		m_matRot *= m::hom_rotation<t_mat, t_vec3>(
			vecCamDir[1], m_phi, 0);

		t_mat matDist = m::hom_translation<t_mat, t_real>(0., 0., -m_dist / m_zoom);
	...
	void UpdatePerspective()
	{
		...
		std::tie(m_matPerspective_inv, std::ignore) =
			m::inv<t_mat, t_vec>(m_matPerspective);
	...
	{
		// viewport
		m_matViewport = m::hom_viewport<t_mat>(
			m_screenDims[0], m_screenDims[1],
			m_z_near, m_z_far);
		std::tie(m_matViewport_inv, std::ignore) =
			m::inv<t_mat, t_vec>(m_matViewport);
...
private:
	...
	t_mat m_mat = m::unit<t_mat>();
	t_mat m_mat_inv = m::unit<t_mat>();

	// rotation and translation matrices
	t_mat m_matRot = m::unit<t_mat>();
	t_mat m_matTrans = m::unit<t_mat>();
	...
	// camera rotation
	t_real m_phi = m::pi<t_real>*t_real(0.25);
	t_real m_theta = -m::pi<t_real>*(0.25);
	t_real m_phi_saved = m::pi<t_real>*t_real(0.25);
	t_real m_theta_saved = -m::pi<t_real>*(0.25);
	...
	t_mat m_matPerspective = m::unit<t_mat>();
	t_mat m_matPerspective_inv = m::unit<t_mat>();
	...
	// screen viewport
	t_mat m_matViewport = m::unit<t_mat>();
	t_mat m_matViewport_inv = m::unit<t_mat>();
